 ...
 */
size_t decode(const unsigned char* const data, const size_t size,
              unsigned char* const output_buffer, const size_t output_buffer_size) {
    size_t written;
    base122_error_t error;

    if(base122_decode(data, size, output_buffer, output_buffer_size, &written, &error) == 0)
        return written;
    else
        return 0;
}
